function step3_alignRefStack(datapath)
...
for k = 1:size(refStack,3)
    [tempAligned,tempShift] = fn_fastAlign(cat(3, alignedOps.suite2pImg(:,:,ops.refIdx),refStack(:,:,k)),'center');
    refStack(:,:,k) = tempAligned(:,:,2);
end 
t = toc; disp(['Time = ' num2str(t) ' sec'])
save([refpath filesep 'refStack.mat'],'refStack','tempShift');
