function plotPSTH(data,mouse,alignStr,stim,closeupFlag)
% plot PSTH function
    probeFlag = true; 
    switch alignStr
        case 'stim'
    ...
    if stim>2; choiceStim = stim-2; else choiceStim = stim; end 
    
    tempMeanColorAxis = cellfun(@(x)(nanmean(x,3)),data.dffStimList,'UniformOutput',false);
    tempMeanColorAxis = max(cellfun(@(x)(prctile(x(:),98)),tempMeanColorAxis));

    figure; climmm = [-tempMeanColorAxis tempMeanColorAxis];
    ...
    for i = 1:length(selAct)
        ...
        if ~probeFlag 
            tempFlag =  {tempBehav(:,5)==stim & tempBehav(:,6)==choiceStim,...
                tempBehav(:,5)==stim & tempBehav(:,6)==3-choiceStim,...
                tempBehav(:,5)==stim & tempBehav(:,6)==0}; 
            subplotNum = 4; 
        else
            tempFlag =  {tempBehav(:,5)==stim & tempBehav(:,6)==choiceStim & tempBehav(:,7)~=2,...
                tempBehav(:,5)==stim & tempBehav(:,6)==choiceStim & tempBehav(:,7)==2,...
                tempBehav(:,5)==stim & tempBehav(:,6)==3-choiceStim & tempBehav(:,7)~=2,...
                tempBehav(:,5)==stim & tempBehav(:,6)==3-choiceStim & tempBehav(:,7)==2,...
                tempBehav(:,5)==stim & tempBehav(:,6)==0 & tempBehav(:,7)~=2,...
                tempBehav(:,5)==stim & tempBehav(:,6)==0 & tempBehav(:,7)==2}; 
            subplotNum = 8; 
        end 
        ...
        for j = 1:length(tempFlag)
            ...
            subplot_tight(subplotNum,length(selAct),i+length(selAct)*(j-1),[0.025 0.015]); 
            if j==1
            ...
            if ~choiceFlag;  xline(tempRT{j}, '-','Color',[0.4 0.4 0.4], 'LineWidth', 1);
            else;  xline(-tempRT{j}, '-','Color',[0.4 0.4 0.4], 'LineWidth', 1); end  
            fn_textOnImg(gca,['n=' int2str(sum(tempFlag{j}))],16 );  xline(0, '-','Color',[0.4 0.4 0.4], 'LineWidth', 1);
            if i~=1; yticks([]); end 
            tempMean{j} = nanmean(nanmean(selAct{i}(:,:,tempFlag{j}),1),3); 
            [tempBias,tempAcc,~,~,acc_L,acc_R] = fn_getAccBias(tempBehav(:,5), tempBehav(:,7)==1,tempBehav(:,6)==0);
            if j==1; temp = {acc_L,acc_R}; title(['acc=' num2str(tempAcc,'%.2f')  ',s' int2str(stim) '=' num2str(temp{choiceStim},'%.2f')]); end 
            if closeupFlag; xlim([-0.6 0.6]); end 

            if probeFlag
                if j==2 
                    tempAct = tempBehav(tempBehav(:,5)==stim & tempBehav(:,7)==2,6); 
    ...
    for i = 1:length(selAct)
        ...
        tempFlag = {tempBehav(:,5)==stim & tempBehav(:,6)==choiceStim,tempBehav(:,5)==stim & tempBehav(:,6)==3-choiceStim,tempBehav(:,5)==stim & tempBehav(:,6)==0};
        for j = 1:length(tempFlag)
    ...
    if ~closeupFlag; tempStr = [strrep(mouse, '_', '__') ', stim = ' int2str(stim) ', aligned to ' alignStr ];
    else; tempStr = [strrep(mouse, '_', '__') ', stim = ' int2str(stim) ', aligned to ' alignStr ', closeup']; end 
    fn_textOnPlot(tempStr, gca, 20);
