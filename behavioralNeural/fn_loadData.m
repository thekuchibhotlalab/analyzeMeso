function [Fnew, beh, Fops,behOps] = fn_loadData(mouse, Fpath,behavPath,behavDates)
% 0.1 -- load F data
%area = 'PPC';
%datapath = ['D:\zz142\committee\110423_111223\green_' area '\suite2p_110623_111223\plane0'];
data = struct();

data = load([Fpath filesep 'Fall.mat']);
...
if ~isfield(data,'F'); data.F = readNPY([Fpath filesep 'F.npy']); end 
%data.stat = readNPY([Fpath filesep 'stat.npy']);
if ~isfield(data,'iscell');data.iscell = readNPY([Fpath filesep 'iscell.npy']); end 
...
% 1 -- Get all sessions
if any(112123==Fops.dayList); Fops.sessionListNum(17:21) = [1 2 3 4 5]; Fops.sessionList(17:21) = {1,2,3,4,5}; end % correct for 112123
