...
%%
mouse = {'zz142','zz151','zz153','zz155','zz159','zz170','zz172','zz173','zz177','zz179','zz180'};
for i = 1:length(mouse)
    ops.mouse = mouse{i};
    ops.datapath = ['G:\ziyi\mesoData\' ops.mouse '_behavior\matlab\'];
    beh = fn_loadBehav(ops);
    dataTable = array2table(beh(:,1:13), 'VariableNames', ...
        {'date','day','session','trialNum', 'stimulus', 'action','responseType','stimulusTime','responseTime',...
        'stimulusFrame','responseFrame','rewardFrame','context'});
    save(['G:\ziyi\mesoData_analyzed\behavior\' mouse{i} '_beh.mat'],'beh');
end 

%%

%% Interleaved summary stats
...
[summaryStatT1,summaryStatT2,summaryStatT1Inter,summaryStatT2Inter,block,blockType] = cellfun(@(x,y)(fn_plotMesoBehavIndvidual(x,y)),mouseList,datapath,'UniformOutput',false);
%%
figure; hold on;
tempAcc = block{4}.acc([3:19 22:49]);
tempType = blockType{4}([3:19 22:49]);
tempColor = [];
for i = 1:length(tempType)
    ...
    plot(i,tempAcc(i),'o','Color',tempColor(i,1:3),'LineWidth',2);
end 
ylim([0.4 1]); yline(0.5,'Color',[0.8 0.8 0.8])

%%
tempAcc = [block{2}.acc([1:47 51:78]); block{3}.acc([1:4 6:50]) ;block{4}.acc([3:19 22:49]); block{6}.acc];
tempAccType = [blockType{2}([1:47 51:78]); blockType{3}([1:4 6:50]) ;blockType{4}([3:19 22:49]); blockType{6}];
tempAccType1 = cellfun(@(x)(strcmp(x,'T1')),tempAccType);
tempAccType2 = cellfun(@(x)(strcmp(x,'T2')),tempAccType);
tempAccType3 = cellfun(@(x)(strcmp(x,'Int')),tempAccType);

figure; 
bar([nanmean(tempAcc(tempAccType1)) nanmean(tempAcc(tempAccType2)) nanmean(tempAcc(tempAccType3))]); ylim([0.4 1]); hold on;
errorbar([nanmean(tempAcc(tempAccType1)) nanmean(tempAcc(tempAccType2)) nanmean(tempAcc(tempAccType3))],...
    [fn_nansem(tempAcc(tempAccType1)) fn_nansem(tempAcc(tempAccType2)) fn_nansem(tempAcc(tempAccType3))], 'LineStyle','none');
%% get summary data
